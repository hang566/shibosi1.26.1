      ...
      return `
        <div class="service-card">
          ...
          <div class="service-actions">
            ...
            ${openBtn}
            ${svc.config?.start_command ? `<button class="btn btn-sm btn-success" onclick="startService('${svc.name}')" ${status === 'online' ? 'disabled' : ''}>▶ 启动</button>` : ''}
            ${svc.config?.stop_command ? `<button class="btn btn-sm btn-danger" onclick="stopService('${svc.name}')" ${status !== 'online' ? 'disabled' : ''}>■ 停止</button>` : ''}
          </div>
        </div>
      `;
    }).join('');
...
async function checkService(name) {
  ...
  if (data.code === 0) {
    ...
    showModal(`服务详情 - ${cfg.name || name}`, `
      <div class="form-group"><label class="form-label">服务名</label><div>${name}</div></div>
      <div class="form-group"><label class="form-label">状态</label><div>${cfg.status || 'unknown'}</div></div>
      <div class="form-group"><label class="form-label">基础URL</label><div>${cfg.base_url || '-'}</div></div>
      <div class="form-group"><label class="form-label">注册时间</label><div>${svc.registered_at || svc.RegisteredAt ? new Date(svc.registered_at || svc.RegisteredAt).toLocaleString('zh-CN') : '-'}</div></div>
    `);
  } else {
